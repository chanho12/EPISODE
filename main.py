...
def episode(data, tag_type, update_method, outputfilename, model, tokenizer, device):
  ...
  for key, value in data.items(): #episode
    ...
    for session in conversations[:5]: #session 
      ...
      #model inference
      utterance = model_response(prompt, tag, last_speaker, model, tokenizer, device)
...
# python main.py --path='chano12/gemma_with_tag' --tag_type='gpt' --update_method='accumulate' --input_file=list_dataset.json --output_file='tag_gemma_accumulate'
...
def main():
  ...
  episode(test_data, tag_type, update_method, my_output_json_file, model, tokenizer, device)
